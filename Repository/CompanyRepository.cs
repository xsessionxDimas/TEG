...
namespace Repository
{
    public class CompanyRepository<T> : RepositoryBase, IUniqueValidation, IRepository<T> where T : BaseEntityObject
    {
        public int SaveRow(T param, string createdBy)
        {
            ...
            {
                SqlCommand cmd = DBClass.GetStoredProcedureCommand("APP_SAVE_NEW_COMPANY");
        ...
        public int UpdateRow(T param, string updatedBy)
        {
            using (DBClass = new MSSQLDatabase())
            {
                ...
                {
                    SqlCommand cmd = DBClass.GetStoredProcedureCommand("APP_UPDATE_COMPANY");
        ...
        public int DeleteRow(int id, string updatedBy)
        {
            ...
            try
            {
                using (DBClass = new MSSQLDatabase())
                {
                    ...
                    {
                        var cmd = DBClass.GetStoredProcedureCommand("APP_DELETE_COMPANY");
        ...
        public IEnumerable<T> FindAll(List<Dictionary<string, object>> keyValueParam)
        {
            ...
            using (DBClass = new MSSQLDatabase())
            {
                SqlCommand cmd = DBClass.GetStoredProcedureCommand("APP_GET_ALL_COMPANY");
        ...
        public T FindbyId(int id)
        {
            ...
            using (DBClass = new MSSQLDatabase())
            {
                SqlCommand cmd = DBClass.GetStoredProcedureCommand("APP_GET_COMPANY_BY_ID");
                cmd.Parameters.AddWithValue("@CompanyId", id);
                var reader = DBClass.ExecuteReader(cmd);
                while (reader.Read())
                {
        ...
        public bool UniqueNameAvailable(List<Dictionary<string, object>> keyValueParam)
        {
            ...
            using (DBClass = new MSSQLDatabase())
            {
                var cmd    = DBClass.GetStoredProcedureCommand("APP_COMPANY_NAME_AVAILABLE");
                RoutinesParameterSetter.Set(ref cmd, keyValueParam);
                var reader = DBClass.ExecuteReader(cmd);
                while (reader.Read())
                {
        ...
        public bool UniqueNameAvailableExcept(List<Dictionary<string, object>> keyValueParam)
        {
            ...
            using (DBClass = new MSSQLDatabase())
            {
                var cmd    = DBClass.GetStoredProcedureCommand("APP_COMPANY_NAME_AVAILABLE2");
                RoutinesParameterSetter.Set(ref cmd, keyValueParam);
                var reader = DBClass.ExecuteReader(cmd);
                while (reader.Read())
                {
