...
namespace Repository
{
    public class CashAdjustmentRepository<T> : RepositoryBase, IRepository<T> where T : BaseEntityObject
    {
        public int SaveRow(T param, string createdBy)
        {
            ...
            {
                SqlCommand cmd = DBClass.GetStoredProcedureCommand("APP_SAVE_NEW_CASH_ADJUSTMENT");
        ...
        public int UpdateRow(T param, string updatedBy)
        {
            using (DBClass = new MSSQLDatabase())
            {
                ...
                {
                    SqlCommand cmd = DBClass.GetStoredProcedureCommand("APP_UPDATE_CASH_ADJUSTMENT");
        ...
        public int DeleteRow(int id, string updatedBy)
        {
            ...
            try
            {
                using (DBClass = new MSSQLDatabase())
                {
                    ...
                    {
                        var cmd = DBClass.GetStoredProcedureCommand("APP_DELETE_CASH_ADJUSTMENT");
                        cmd.Parameters.AddWithValue("@CashAdjustmentId", id);
                        cmd.Parameters.AddWithValue("@LastUpdatedBy", updatedBy);
                        DBClass.ExecuteNonQuery(cmd, txn);
                        txn.Commit();
                    }
        ...
        public IEnumerable<T> FindAll(List<Dictionary<string, object>> keyValueParam)
        {
            ...
            using (DBClass = new MSSQLDatabase())
            {
                SqlCommand cmd  = DBClass.GetStoredProcedureCommand("APP_GET_ALL_CASH_ADJUSTMENT");
        ...
        public T FindbyId(int id)
        {
            ...
            using (DBClass = new MSSQLDatabase())
            {
                SqlCommand cmd = DBClass.GetStoredProcedureCommand("APP_GET_CASH_ADJUSTEMENT_BY_ID");
                cmd.Parameters.AddWithValue("@CashAdjustmentId", id);
                var reader     = DBClass.ExecuteReader(cmd);
                while (reader.Read())
                {
        ...
        public DataSet[] GetReportData(int adjustmentId, string inWords)
        {
            ...
            using (DBClass = new MSSQLDatabase())
            {
                SqlCommand cmd         = DBClass.GetStoredProcedureCommand("REPORT_GET_CASH_ADJUSTMENT_DATA");
        ...
        public string GetVoucherCode()
        {
            ...
            using (DBClass = new MSSQLDatabase())
            {
                SqlCommand cmd = DBClass.GetStoredProcedureCommand("GETCASHADJUSTMENTCODENUMBER");
                var reader = DBClass.ExecuteReader(cmd);
                while (reader.Read())
                {
